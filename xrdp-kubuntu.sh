...
show_dry_run_summary() {
    ...
    if [ "$OPTIMIZE_PERFORMANCE" = true ]; then
        print_msg "⚡ PERFORMANCE OPTIMIZATIONS:" "$GREEN"
    ...
    if [ "$CONFIGURE_KDE" = true ]; then
        print_msg "🖥️  CONFIGURE KDE/PLASMA:" "$GREEN"
    ...
    if [ "$FIX_NETWORK_AUTH" = true ]; then
        print_msg "🔓 FIX NETWORK AUTHENTICATION:" "$GREEN"
    ...
    if [ "$SET_SOLID_BG" = true ]; then
        print_msg "🎨 SET SOLID BACKGROUND ($BG_COLOR):" "$GREEN"
    ...
    if [ "$DISABLE_ENCRYPTION" = true ]; then
        print_msg "🔐 DISABLE ENCRYPTION:" "$GREEN"
    ...
    if [ "$PERSISTENT_SESSIONS" = true ]; then
        print_msg "💾 PERSISTENT SESSIONS:" "$GREEN"
